...
user_fav_rdd = dpark.textFile(files, splitSize=64<<20).map(
            ...
            # uid, tid, sid
            lambda val_list: len(val_list)==3 and all([val.isdigit() for val in val_list])
        ).map(
...
# track -> artist info
track_artist_dict = dpark.textFile('/mfs/alg/radio/song_info_v2.data', splitSize=4<<20).map(
            lambda line: line.strip().split('\t')
        ).map(
            # tid, new_id, aids, sids, main_subject_id, is_hidden, artist_name, song_name
            lambda val_list: (int(val_list[1]), [int(aid) for aid in val_list[2].split(',')] if val_list[2] else [])
        ).collectAsMap()
dtrack_artist_dict = dpark.broadcast(track_artist_dict)

# user ban artists
def ban_artist(uid, ban_track_list):
    fav_artist_set = set([int(tid) for tid, _ in RadioUser.get_user_favorite_artists(uid)])
    ban_artist_cnt_dict = {}
...
user_ban_rdd = dpark.textFile(files, splitSize=64<<20).map(
            ...
            # uid, sid, timestamp
            lambda val_list: len(val_list)==3 and all([val.isdigit() for val in val_list[:2]]) \
                    and (int(val_list[0]) in dseed_user_set.value)
        ).map(
...
# banned artist tracks
banned_artist_list = user_ban_rdd.flatMap(
            lambda (uid, ban_artist_set): ban_artist_set
        ).uniq().collect()
banned_artist_set = set(banned_artist_list)
output_file = open('/mfs/user/wuhong/Data/fm/artist_track_dict.csv', 'w')
...
# banned tracks
def _(fav_track_list, ban_artist_set):
...
user_banned_rdd = user_fav_rdd.outerJoin(user_ban_rdd, 100).mapValue(
            lambda (fav_track_list, ban_artist_set): _(fav_track_list, ban_artist_set)
        ).map(
            lambda (uid, res_list): '%s:%s' % (uid, '|'.join(res_list))
        ).saveAsCSVFile('/mfs/user/wuhong/tmp/blacklst')
